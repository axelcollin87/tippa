...
export default async function LeagueRoomPage(props: { params: Promise<{ id: string }> }) {
  ...
  if (isGlobal) {
    const allUsers = await prisma.user.findMany({
      select: {
        ...
        totalScore: true,
        matchBets: {
          select: { matchId: true, pointsAwarded: true, pointsAwardedProgress: true }
        }
      }
    });
    ...
    membersList = allUsers.map(u => ({ id: `global-member-${u.id}`, userId: u.id, user: u }));
  } else {
  ...
  const sortedMembers = [...membersList].sort((a, b) => b.user.totalScore - a.user.totalScore);
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <Link href="/" className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors font-bold text-sm uppercase tracking-widest">
      ...
      <div className={`bg-card border ${isGlobal ? 'border-primary/50' : 'border-border'} rounded-[2rem] p-8 md:p-12 relative overflow-hidden shadow-2xl`}>
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-2">
             <span className={`text-[10px] font-black ${isGlobal ? 'bg-primary/20 text-primary' : 'bg-primary text-primary-foreground'} px-3 py-1 rounded-full uppercase tracking-[0.2em]`}>
          ...
          <div className="flex items-center gap-4 text-muted-foreground font-medium">
            {!isGlobal && (
              <div className="flex items-center gap-1.5">
                ...
                <span>Admin: <span className="text-foreground font-bold">{leagueData.admin.name}</span></span>
              </div>
            )}
            <div className="flex items-center gap-1.5">
              <Users size={16} />
      ...
      <div className={`grid grid-cols-1 ${isGlobal ? 'lg:grid-cols-1 max-w-4xl mx-auto' : 'lg:grid-cols-3'} gap-8`}>
        ...
        <div className={`${isGlobal ? '' : 'lg:col-span-2'} space-y-12`}>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-black text-foreground uppercase tracking-tight">Formkurva</h2>
              <InfoPopover title="Poängutveckling">
                <p>Grafen visar medlemmarnas kumulativa poäng match för match (i kronologisk ordning).</p>
                {isGlobal && <p className="mt-2 text-xs italic">Eftersom detta är den globala tabellen visas endast de 10 bästa spelarna i grafen.</p>}
              </InfoPopover>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-2xl font-black text-foreground uppercase tracking-tight">Leaderboard</h2>
              {!isGlobal && (
                <InfoPopover title="Globala Poäng">
                  <p>Observera att denna tabell visar medlemmarnas <b>totala globala poäng</b>. Dina poäng är desamma i alla ligor du är med i.</p>
                  <p className="mt-2 text-xs italic">Syftet med ligan är att jämföra dina globala poäng mot just dessa vänner.</p>
                </InfoPopover>
              )}
            </div>
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-border">
                  {sortedMembers.map((member, index) => {
                    ...
                    const pos = index + 1;
                    return (
                      <tr key={member.id} className={`${isMe ? 'bg-primary/5' : ''} transition-colors hover:bg-secondary/30`}>
                        <td className="px-6 py-5">
                          <span className={`flex items-center justify-center w-8 h-8 rounded-lg font-black text-sm ${
                            pos === 1 ? 'bg-yellow-500 text-white' : 
                            pos === 2 ? 'bg-slate-300 text-slate-700' : 
                            pos === 3 ? 'bg-amber-600 text-white' : 'text-muted-foreground'
                          }`}>
                            {pos}
                          </span>
                        </td>
                        <td className="px-6 py-5">
                          <div className="font-bold text-foreground flex items-center gap-2">
                            {member.user.name}
                            {isMe && <span className="text-[9px] bg-primary/20 text-primary px-1.5 py-0.5 rounded uppercase">Du</span>}
                          </div>
                        </td>
                        <td className="px-6 py-5 text-center font-bold text-muted-foreground">
                          {member.user.matchBets.length}
                        </td>
                        <td className="px-6 py-5 text-right">
                          <span className="font-black text-lg text-primary">{member.user.totalScore}</span>
                        </td>
                      </tr>
                    );
                  })}
